...
func (h *userHandler) GetUsers(c *gin.Context) {
	...
	response := helper.ApiResponse("Detail Users", http.StatusOK, "success", formatter.FormatUsers(users))
	c.JSON(http.StatusOK, response)
}
...
func (h *userHandler) CreateUser(c *gin.Context) {
	...
	if err != nil {
		...
		response := helper.ApiResponse("Create Userfailed", http.StatusBadRequest, "error", errorMessage)
		c.JSON(http.StatusBadRequest, response)
		return
	}
	newUser, err := h.service.UserServiceCreate(input)
	if err != nil {
		response := helper.ApiResponse("Create Userfailed", http.StatusBadRequest, "error", nil)
		c.JSON(http.StatusBadRequest, response)
		return
	}
	response := helper.ApiResponse("Create User", http.StatusOK, "success", formatter.FormatUser(newUser))
	c.JSON(http.StatusOK, response)
}
func (h *userHandler) UpdateUser(c *gin.Context) {
	...
	if err != nil {
		errors := helper.FormatValidationError(err)
		errorMessage := gin.H{"errors": errors}
		response := helper.ApiResponse("Update Userfailed", http.StatusBadRequest, "error", errorMessage)
		c.JSON(http.StatusBadRequest, response)
		return
	}
	...
	response := helper.ApiResponse("Update User", http.StatusOK, "success", formatter.FormatUser(updatedUser))
	c.JSON(http.StatusOK, response)
}
